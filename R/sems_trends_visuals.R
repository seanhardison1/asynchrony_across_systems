...
m2 <- lm(cpe_bio ~ sae_bio, data = ndf_va)
s <- DHARMa::simulateResiduals(m2, n = 1000);plot(s)
acf(residuals(m2))
...
m3 <- lm(sae_land ~ sae_bio,
         data = ndf_va)
s <- DHARMa::simulateResiduals(m3, n = 1000);plot(s)
acf(residuals(m3))
...
m10 <- lm(sae_land ~ sae_bio + land_ratio
          , data = ndf_md)
summary(m10)
...
## MD 2 -------
m11 <- lm(s_land ~ s_land_short, data = ndf_md)
# m11 <- lm(s_land ~ s_land_short, data = ndf_md)
...
ggplot(ndf_md) + 
  geom_point(aes(y = s_land, x = s_land_short))

# ggplot(ndf_md) + 
#   geom_point(aes(y = s_land, x=  s_land_short2))
...
## Trends----
bc <- pred_out_ann %>% filter(common == "blue crab",
                              metacomm == "VA")

mod_bc1 <- lm(est ~ year, data = bc)
s <- DHARMa::simulateResiduals(mod_bc1, 1000); plot(s)
acf(residuals(mod_bc1))
...
fill_vec <- setNames(ggsci::pal_d3("category20")(n = 7),unique(c(va_specs, 
                                                                 md_specs)))
annual_biomass <- 
...
land_shift_df <- 
  va_landings %>% 
    filter(species %in% c("spot", "Atlantic croaker",
                          "striped bass", "blue crab")) %>% 
    right_join(.,expand_grid(month = 1:12,
                             species = c("Atlantic croaker", "spot", "striped bass","blue crab"),
                             year = 2002:2018)) %>% 
...
bc_shift <- gam(weighted_month ~ s(year), data = land_shift_df %>% 
                  filter(species == "blue crab"))
s <- DHARMa::simulateResiduals(bc_shift, n = 1000);plot(s)
summary(bc_shift)
acf(residuals(bc_shift))
draw(bc_shift)
...
# blue crab
bc_shift_cpe <- lm(cpe_land ~ weighted_month, data = shift_cpe %>% 
                     filter(species == "blue crab"))
s <- DHARMa::simulateResiduals(bc_shift_cpe, n = 1000);plot(s)
acf(residuals(bc_shift_cpe))
summary(bc_shift_cpe)
...
# blue crab
bc_cpue_mod <- lm(cpue ~ year, data = va_cpue %>% 
                    filter(species == "blue crab"))
summary(bc_cpue_mod)
acf(residuals(bc_cpue_mod))
...
# md_bio_plt <- 
  pred_out_seas %>% 
    ...
    geom_line(aes(y = est, x = month, color = year,
                  group = year)) +
    geom_errorbar(aes(ymin = lwr, ymax = upr, color = year,
                      group = year, x = month),
                  width = 0) +
...
pred_out_seas %>% 
  ...
  geom_errorbar(aes(ymin = lwr, ymax = upr, x = ymon),width = 0) +
  scale_y_log10(labels = function(x) format(x, scientific = FALSE, trim = TRUE)) +
  facet_wrap(~common, scales = "free_y") +
  labs(x = "Month",
       y = labs(y = "Biomass (&times;10<sup>5</sup> kg)"),
...
pred_out_seas %>% 
  ...
  mutate(year = factor(year, levels = 2002:2018)) %>% 
  ggplot() +
  geom_line(aes(y = est, x = ymon, color = common)) +
  geom_errorbar(aes(ymin = lwr, ymax = upr, color = common, x = ymon),
                width = 0)
