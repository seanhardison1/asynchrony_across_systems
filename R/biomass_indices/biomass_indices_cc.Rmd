---
title: "Channel catfish biomass index development"
output:
...
selected_species <- "channel catfish"
ss <- "cc"
...
```{r}
  hurd_mod <- sdmTMB(biomass ~ 0 +
                         season +
                         year2 +
                         s(month, year2, bs = 'fs', k = 4),
...
pred_out_seas <- do.call(rbind, index_md) %>% 
...
## save outputs
pred_out_seas_cc <- pred_out_seas %>% 
  as_tibble()
save(pred_out_seas_cc,
