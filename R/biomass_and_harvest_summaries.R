...
va_landings %>% 
  ...
  group_by(species) %>% 
  dplyr::summarise(real_total_value = sum(real_total_value)) %>% 
  arrange(desc(real_total_value)) %>% 
  filter(!species %in% c("blue crab","menhaden")) %>% 
  mutate(rank = 1:nrow(.),
         harv_cum = cumsum(real_total_value)/sum(real_total_value)) 
...
md_landings %>% 
  ...
  dplyr::summarise(real_total_value = sum(real_total_value)) %>% 
  arrange(desc(real_total_value)) %>% 
  filter(!species %in% c("blue crab","Atlantic menhaden")) %>% 
  mutate(rank = 1:nrow(.),
         harv_cum = cumsum(real_total_value)/sum(real_total_value)) 
